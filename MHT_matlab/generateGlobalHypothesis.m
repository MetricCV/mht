function [bestHypothesis bestScore trackIndexInTrees selectedTrackIDs] = generateGlobalHypothesis(scoreTreeSet, idTreeSet, incompabilityListTreeSet, clusters, ICL_clusters, selectedTrackIDsPrev, other_param)
...
for i = 1:clustersNo
   ...
   graph_density = edge_num/(size(weightMat,2)*(size(weightMat,2)+1)/2);
   ...
   maxCliqueNo = 1000;  
   if thLineFunc(max(200,min(1000,size(weightMat,2)))-199) < graph_density && size(weightMat,2) > 80   
       % Qualex-ms
